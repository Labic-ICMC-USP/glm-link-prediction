...
DATA_COLLECTION_SCRIPT = "graph_creation/collect_news.py"
EXTRACTION_SCRIPT = "graph_creation/extract_5W1H_from_news.py"
GRAPH_CONSTRUCTION_SCRIPT = "graph_creation/generate_event_graph.py"
...
def main():
    ...
    if not args.skip_collection:
        if os.path.exists(news_output_folder) and not args.overwrite:
            ...
            os.system(f"python {DATA_COLLECTION_SCRIPT} --config {args.collection_config} --output_folder {news_output_folder}")
    ...
    if not args.skip_extraction:
        if os.path.exists(extraction_output_file) and not args.overwrite:
            ...
            os.system(f"python {EXTRACTION_SCRIPT} --config {args.extraction_config} --input_folder {news_output_folder} --output_file {extraction_output_file}")
    ...
    if not args.skip_construction:
        if os.path.exists(graph_output_file) and not args.overwrite:
            ...
            os.system(f"python {GRAPH_CONSTRUCTION_SCRIPT} --config {args.graph_config} --input_file {extraction_output_file} --output_file {graph_output_file}")
